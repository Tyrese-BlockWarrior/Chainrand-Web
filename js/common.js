...
var lazyTippy = (function () {
	...
	var loadTippy = function () {
		...
		$.ajax({
			...
			success: function () {
				$.ajax({
					url: "https://unpkg.com/tippy.js@6", 
					cached: true, 
					dataType: "script",
				})
			}
	...
	var self = function () {
		...
		var e = function () {
			if (window.tippy) {
				window.tippy.apply(undefined, a);
			} else if (nTries < 30) {
				loadTippy();
				nTries++;
				setTimeout(e, 1000);
...
var walletConnector = (function () {
	...
	self.sync = function (key) {
		...
		var supportedNetworksCtx = ['div', {class: 'tippy-supported'}, 'Supported Networks:'];
		for (var i = 0; i < chainlist.length; ++i) {
			if (chainlist[i].supported) {
				supportedNetworksCtx.push(['div', '&#8226; ' + capitalize(chainlist[i].name)]);
			}
		}
		lazyTippy('#' + id + ' .info', {
			content: HTML(supportedNetworksCtx), 
			allowHTML: true, 
			placement: 'bottom'
		});
	}
...
var addTokenTippys = function (resultsId, numTries) {

	var addTippy = function (name, content) {
		lazyTippy('#' + resultsId + ' .value[name="' + name + '"]', {
			content: content 
		});		
	}

	addTippy('minter', "Address of the Minter.");
	addTippy('verified', "Whether the project has been verified by Chainrand.");
	addTippy('codeURI', "The URI of the code's zip.");
	addTippy('codeHash', "The SHA-256 hash of the code's zip in hex.");
	addTippy('seedKey', "The seed key used by the code.");
	addTippy('seedKeyHash', "The SHA-256 hash of the seed key in hex.");
	addTippy('randomness', "The randomness generated by the Chainlink VRF.");
	addTippy('paid', "The total nominal fee paid to Chainrand in the native currency of the chain.");
	addTippy('openSeaURI', "The OpenSea link, which can also be used as the project's provenance.");
	addTippy('projectURI', "The IPFS CID or URL of the project.");
	addTippy('imageURI', "The IPFS CID or URL of the Chainrand NFT image. " +
		"If none is provided, a on-chain random SVG will be generated.");
}